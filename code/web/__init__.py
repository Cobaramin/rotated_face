...
def upload_file():
    ...
    if file_stream and allowed_file(file_stream.filename):
        img = cv2.imdecode(np.fromstring(file_stream.read(), np.uint8), cv2.IMREAD_UNCHANGED)
        ...
        try:
            ...
            pass
        output.seek(0)
        return send_file(output, as_attachment=True, attachment_filename=file_stream.filename, mimetype='image/jpeg')
    else:
